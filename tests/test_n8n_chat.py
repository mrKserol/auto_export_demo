...
from app.services.compact_action_token import consume_compact_action_token
...
class N8NChatHandlerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_intake_card_reply_creates_two_keyboard_rows(self) -> None:
        reply = {
            "type": "intake_card",
            "text": "Распознанные данные клиента",
            "session_id": "11111111-1111-1111-1111-111111111111",
            "keyboard": [
                {"type": "web_app", "text": "✏️ Ручная коррекция", "url": "https://api.ulkar.ru/miniapp/customer?token=x"},
                {"type": "callback", "text": "➕ Добавить клиента", "action": "intake.add_client"},
            ],
        }
        with patch(
            "app.handlers.n8n_chat.N8NChatService.send_telegram_command",
        ...
        action = consume_compact_action_token(
            callback_data.split(":", 1)[1], action="intake.add_client",
            channel="telegram", external_user_id="42", conversation_id="42",
        )
        self.assertEqual(action.action, "intake.add_client")
        self.assertEqual(str(action.session_id), "11111111-1111-1111-1111-111111111111")
