...
@dataclass(frozen=True)
class BatchCustomerContextToken:
    purpose: str
    batch_id: int
    telegram_user_id: int
    origin_chat_id: int
...
def create_customer_batch_context_token(
    *,
    secret: str,
    batch_id: int,
    telegram_user_id: int,
    ...
    current_time = int(time.time() if now is None else now)
    payload = BatchCustomerContextToken(
        purpose=PURPOSE_CREATE_CUSTOMER_FROM_BATCH,
...
def verify_customer_batch_context_token(
    token: str,
    *,
    secret: str,
    expected_telegram_user_id: int | None = None,
    now: int | None = None,
) -> BatchCustomerContextToken:
    ...
    if not hmac.compare_digest(provided_signature, expected_signature):
        raise TokenError(
            ...
            "Подпись контекстного токена недействительна",
        )
    ...
    if payload.get("purpose") != PURPOSE_CREATE_CUSTOMER_FROM_BATCH:
        raise TokenError("INVALID_CONTEXT_TOKEN", "Неверное назначение токена")

    try:
        context = BatchCustomerContextToken(
    ...
    if context.exp < current_time:
        raise TokenError(
            ...
            "Срок действия контекстного токена истёк",
        )
